...
) -> AsyncGenerator[AsyncSession, None]:  # <--- 修正 3: 修正返回类型注解
    """
    提供一个已经填充了测试数据的数据库会话。
    """
    async with db_session_factory() as session:
        threads_to_create = [
            Thread(
                ...
                first_message_excerpt="这是一个关于百合破坏的话题",
            ),
            Thread(
                channel_id=1,
                thread_id=102,
            ...
            Thread(
                ...
                author_id=3,
                created_at=datetime.now(),
                first_message_excerpt="推荐一些好看的小说",
            ),
            Thread(
                channel_id=1,
                thread_id=104,
                ...
                author_id=4,
                created_at=datetime.now(),
                first_message_excerpt="社区规则，禁止讨论百合破坏",
            ),
        ]
...
async def test_search_with_excluded_keywords_and_exemptions(
    ...
    query = ThreadSearchQuery(
        exclude_keywords="百合破坏", exclude_keyword_exemption_markers=["禁", "🈲"]
    )
