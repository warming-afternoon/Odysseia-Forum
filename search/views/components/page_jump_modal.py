...
class PageJumpModal(discord.ui.Modal, title="跳转到页面"):
    def __init__(self, max_page: int, submit_callback: Callable[[discord.Interaction, int], Coroutine]):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if 1 <= page_num <= self.max_page:
                ...
                await interaction.followup.send(f"页码必须在 1 到 {self.max_page} 之间。", ephemeral=True)
        except ValueError:
