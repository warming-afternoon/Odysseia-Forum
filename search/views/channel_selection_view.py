...
class ChannelSelectionView(discord.ui.View):
    ...
    def __init__(self, cog: "Search", original_interaction: discord.Interaction, channels: List[discord.ForumChannel], all_channel_ids: List[int]):
        ...
        options.extend([discord.SelectOption(label=ch.name, value=str(ch.id)) for ch in channels[:24]])
    ...
    async def on_channel_select(self, interaction: discord.Interaction):
        ...
        if "all" in selected_values:
            ...
            selected_names = [ch.name for ch in self.channels if str(ch.id) in selected_values]
    ...
    async def on_confirm(self, interaction: discord.Interaction):
        ...
        # 切换到通用搜索视图
        generic_view = GenericSearchView(self.cog, interaction, self.selected_channel_ids)
