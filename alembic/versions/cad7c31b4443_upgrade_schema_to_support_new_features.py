...
def upgrade() -> None:
    ...
    op.add_column(
        "mutex_tag_group",
        sa.Column("override_tag_name", sqlmodel.AutoString(), nullable=True),
    )
    op.create_index(
        op.f("ix_mutex_tag_group_override_tag_name"),
        "mutex_tag_group",
        ["override_tag_name"],
        unique=False,
    )
    ...
    op.add_column(
        "thread",
        sa.Column("display_count", sa.BigInteger(), server_default="0", nullable=False),
    )
    op.add_column(
        "thread",
        sa.Column("show_flag", sa.Boolean(), server_default="1", nullable=False),
    )
    op.create_index(
        op.f("ix_thread_created_at"), "thread", ["created_at"], unique=False
    )
    op.create_index(
        op.f("ix_thread_display_count"), "thread", ["display_count"], unique=False
    )
    op.create_index(
        op.f("ix_thread_not_found_count"), "thread", ["not_found_count"], unique=False
    )
    op.create_index(op.f("ix_thread_show_flag"), "thread", ["show_flag"], unique=False)

    op.add_column(
        "usersearchpreferences",
        sa.Column("created_after", sqlmodel.AutoString(), nullable=True),
    )
    op.add_column(
        "usersearchpreferences",
        sa.Column("created_before", sqlmodel.AutoString(), nullable=True),
    )
    op.add_column(
        "usersearchpreferences",
        sa.Column("active_after", sqlmodel.AutoString(), nullable=True),
    )
    op.add_column(
        "usersearchpreferences",
        sa.Column("active_before", sqlmodel.AutoString(), nullable=True),
    )
    op.add_column(
        "usersearchpreferences",
        sa.Column(
...
def downgrade() -> None:
    ...
    op.add_column(
        "usersearchpreferences", sa.Column("after_date", sa.DATETIME(), nullable=True)
    )
    op.add_column(
        "usersearchpreferences", sa.Column("before_date", sa.DATETIME(), nullable=True)
    )
    ...
    # --- 恢复 MutexTagGroup 表 ---
    op.drop_index(
        op.f("ix_mutex_tag_group_override_tag_name"), table_name="mutex_tag_group"
    )
