...
class TagVoteView(discord.ui.View):
    """标签投票视图"""
    def __init__(self, thread_id: int, thread_name: str, tag_map: dict[int, str], api_scheduler, session_factory):
    ...
    def create_buttons(self):
        ...
        for i, (tag_id, tag_name) in enumerate(tags):
            self.add_item(TagVoteButton(tag_id, tag_name, vote_value=1, row=i // 5, callback=self.handle_vote))
        
        # 点踩按钮
        base_row = (len(tags) - 1) // 5 + 1
        for i, (tag_id, tag_name) in enumerate(tags):
            self.add_item(TagVoteButton(tag_id, tag_name, vote_value=-1, row=base_row + (i // 5), callback=self.handle_vote))

    async def handle_vote(self, button: TagVoteButton, interaction: discord.Interaction):
        """处理按钮点击的核心逻辑"""
        try:
            ...
            error_coro = interaction.followup.send("处理您的评价时发生错误，请稍后再试。", ephemeral=True)
    ...
    def create_embed(self, stats: dict) -> discord.Embed:
        ...
        """
        embed = discord.Embed(
            title=f"帖子 “{self.thread_name}” 的标签评价",
