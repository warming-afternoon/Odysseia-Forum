...
class BatchUncollectView(BaseManagementView):
    ...
    async def _confirm_uncollect_all(self, interaction: discord.Interaction, button: discord.ui.Button):
        """确认取消全部收藏的回调"""
        ...
        if not thread_ids:
            await interaction.response.edit_message(content="没有帖子可以取消收藏。", view=None)
            return
            
        async with self.cog.get_collection_service() as service:
            result = await service.remove_collections(interaction.user.id, thread_ids)
            await interaction.response.edit_message(
                content=f"操作完成！成功取消收藏 {result['removed']} 个帖子。", view=None
            )
    ...
    @discord.ui.button(label="取消全部收藏", row=1, style=discord.ButtonStyle.danger, custom_id="uncollect_all")
    async def uncollect_all_button(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    @discord.ui.button(label="取消选中收藏", row=1, style=discord.ButtonStyle.primary, custom_id="uncollect_selected")
    async def uncollect_selected_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if not self.selected_threads:
            await interaction.response.send_message("请先选择帖子。", ephemeral=True, delete_after=10)
            return
