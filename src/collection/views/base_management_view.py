...
class BaseManagementView(discord.ui.View):
    ...
    def build_embed(self) -> discord.Embed:
        ...
        embed.set_footer(text=f"第 {self.page}/{self.max_page} 页 | 共 {self.total_items} 个帖子")
        return embed
    ...
    async def update_view(self, interaction: discord.Interaction):
        ...
        self.page_jump_button.style = discord.ButtonStyle.primary if self.max_page > 1 else discord.ButtonStyle.secondary
    ...
    def build_selection_embed(self) -> discord.Embed:
        ...
        for idx, thread_id in enumerate(sorted(list(self.selected_threads)), start=1):
            ...
            selected_thread_lines.append(f"{idx}. [{title[:80]}]({url})") # 避免描述过长
    ...
    async def on_selected_threads_change(self, interaction: discord.Interaction, current_page_selection: Set[str]):
    ...
    @discord.ui.button(label="⬅️ 上一页", custom_id="prev_page", row=2, style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(label="1/1", custom_id="page_jump", row=2, style=discord.ButtonStyle.primary)
    ...
    @discord.ui.button(label="下一页 ➡️", custom_id="next_page", row=2, style=discord.ButtonStyle.secondary)
