...
class BatchCollectView(BaseManagementView):
    ...
    @discord.ui.button(label="收藏全部帖子", row=1, style=discord.ButtonStyle.success, custom_id="collect_all")
    async def collect_all_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not thread_ids:
            await interaction.response.send_message("没有帖子可以收藏。", ephemeral=True, delete_after=10)
            return

        async with self.cog.get_collection_service() as service:
            result = await service.add_collections(interaction.user.id, thread_ids)
    ...
    @discord.ui.button(label="收藏选中帖子", row=1, style=discord.ButtonStyle.primary, custom_id="collect_selected")
