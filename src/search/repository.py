...
from sqlmodel import select, func, or_, and_, case, cast, Float, text
from sqlalchemy import table, column, insert, delete, Integer
...
class SearchRepository:
    ...
    def _apply_search_filters(self, query, statement, resolved_include_tag_ids, resolved_exclude_tag_ids):
        # ... (代码不变)
    ...
    def _apply_ranking(self, statement, resolved_include_tag_ids):
        ...
        if resolved_include_tag_ids:
            ...
            wilson_score_expr = case(
                (total_votes > 0, (p_hat + z_squared / (2 * total_votes) - z * func.sqrt((p_hat * (1 - p_hat) + z_squared / (4 * total_votes)) / total_votes)) / (1 + z_squared / total_votes)),
                else_=0.0,
            )
            tag_weight = case((total_votes > 0, wilson_score_expr), else_=RankingConfig.DEFAULT_TAG_SCORE)
        else:
        ...
        penalty_factor = case(
            (and_(tag_weight < RankingConfig.SEVERE_PENALTY_THRESHOLD, total_votes >= RankingConfig.SEVERE_PENALTY_MIN_VOTES), RankingConfig.SEVERE_PENALTY_FACTOR),
            (and_(tag_weight < RankingConfig.MILD_PENALTY_THRESHOLD, total_votes >= RankingConfig.MILD_PENALTY_MIN_VOTES), RankingConfig.MILD_PENALTY_FACTOR),
            else_=1.0,
    ...
    async def search_threads_with_count(
        ...
        """
        根据搜索条件搜索帖子并分页。(应用层物化+高性能最终版)
        """
        try:
            # --- 步骤 0: 解析标签 ---
            resolved_include_tag_ids = [id for name in query.include_tags for id in self.tag_service.get_ids_by_name(name)]
            resolved_exclude_tag_ids = [id for name in query.exclude_tags for id in self.tag_service.get_ids_by_name(name)]
            ...
            candidate_ids_stmt = self._apply_search_filters(
                query, candidate_ids_stmt, resolved_include_tag_ids, resolved_exclude_tag_ids
            )
            result = await self.session.execute(candidate_ids_stmt)
            final_ids = set(result.scalars().all())

            if not final_ids: return [], 0
            ...
            if query.keywords:
                ...
                for group in and_groups:
                    ...
                    if or_keywords:
                        fts_filters.append(func.match(thread_fts_table.c.thread_fts, " OR ".join(or_keywords)))
                
                if fts_filters:
                    include_stmt = select(thread_fts_table.c.rowid).where(and_(*fts_filters))
            ...
            if query.exclude_keywords:
                exemption_markers = query.exclude_keyword_exemption_markers or ["禁", "🈲"]
                exclude_keywords_list = [kw.strip() for kw in re.split(r"[,，/\s]+", query.exclude_keywords) if kw.strip()]
                
                # VITAL PERFORMANCE FIX: Build a single MATCH query for all excluded keywords
                all_exclude_parts = []
                for keyword in exclude_keywords_list:
                    exemption_clauses = [f'NEAR("{keyword}" "{marker}", 5)' for marker in exemption_markers]
                ...
                if all_exclude_parts:
                    ...
                    exclude_stmt = select(thread_fts_table.c.rowid).where(text("match(thread_fts, :expr)").bindparams(expr=final_exclude_expr))
            ...
            # --- 步骤 5: 获取分页数据 ---
            if not final_ids: return [], total_count
            ...
            if query.sort_method == "comprehensive":
                statement_to_execute, final_score_expr = self._apply_ranking(statement_to_execute, resolved_include_tag_ids)
                order_by = final_score_expr.desc() if query.sort_order == "desc" else final_score_expr.asc()
            else:
                sort_col_name = query.sort_method if hasattr(Thread, query.sort_method) else "last_active_at"
                sort_col = getattr(Thread, sort_col_name)
                order_by = sort_col.desc() if query.sort_order == "desc" else sort_col.asc()
