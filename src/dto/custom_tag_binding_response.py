...
class CustomTagBindingResponse(BaseModel):
    ...
    category: int = Field(description="分类枚举：1=癖好，2=作品，3=角色，4=特质，5=情节，6=背景，7=玩法")
    """标签分类整数值"""

    category_name: str = Field(description="分类枚举对应的中文名称")
    """标签分类名称"""

    source: Literal["custom"] = Field(description="标签来源，固定为 custom")
    """自定义标签来源标识"""

    enabled: bool = Field(description="标签是否启用；停用标签的现有绑定仍可展示")
    """标签池中的启用状态"""

    binding_id: str = Field(description="当前绑定轮次 ID，以十进制字符串返回；重新添加产生新轮次")
    """当前标签绑定轮次 ID"""
