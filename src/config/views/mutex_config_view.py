...
class MutexConfigView(discord.ui.View):
    def __init__(self, handler: "MutexTagsHandler", all_tag_names: List[str],origin_interaction : discord.Interaction):
    ...
    def update_components(self):
        ...
        add_button = discord.ui.Button(label="➕ 新增", style=discord.ButtonStyle.primary)
        add_button.callback = self.on_add_button_click
        self.add_item(add_button)

        delete_button = discord.ui.Button(label="➖ 删除", style=discord.ButtonStyle.danger)
        delete_button.callback = self.on_delete_button_click
        self.add_item(delete_button)

        close_button = discord.ui.Button(label="🔒 关闭", style=discord.ButtonStyle.secondary)
    ...
    async def on_add_button_click(self, interaction: discord.Interaction):
        ...
        add_view = AddMutexGroupView(self.handler, self.all_tag_names,self.origin_interaction)
        await self.handler.bot.api_scheduler.submit(
            coro_factory=lambda: interaction.followup.send(
                embed=add_view.build_embed(),
                view=add_view,
                ephemeral=True
            ),
            priority=1,
