...
class DeleteGroupModal(discord.ui.Modal, title="删除互斥组"):
    def __init__(self, handler: "MutexTagsHandler", view: "MutexConfigView"):
        ...
        self.view = view
        self.group_id_input = discord.ui.TextInput(label="输入要删除的互斥组ID", placeholder="例如: 1")
        self.add_item(self.group_id_input)

    async def on_submit(self, interaction: discord.Interaction):
        await self.handler.process_delete_modal(interaction, self.group_id_input.value, self.view)