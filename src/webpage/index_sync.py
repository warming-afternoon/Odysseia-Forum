...
class IndexSyncService:
    ...
    def clear_user_cache(self, user_id: int):
        ...
        keys_to_remove = [
            key
            for key in self.username_cache.keys()
    ...
    async def convert_thread_to_json(self, thread: Thread) -> Dict[str, Any]:
        ...
        try:
            channel = self.bot.get_channel(
                thread.channel_id
            ) or await self.bot.fetch_channel(thread.channel_id)
            if channel and hasattr(channel, "guild") and channel.guild:
                author_name = await self.get_user_nickname(
                    channel.guild.id, thread.author_id
                )
        except Exception as e:
    ...
    async def update_config_js(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"成功更新 config.js: guild_id={guild_id}, channels={len(channels)}个"
            )
    ...
    async def upload_to_workers_kv(self) -> None:
        ...
        try:
            ...
            # 验证配置
            if not all([api_token, account_id]) or not (
                kv_namespace_id or kv_namespace
            ):
            ...
            if not wrangler_cmd:
                logger.error(
                    "未找到 wrangler 命令，请确保已安装: npm install -g wrangler"
                )
            ...
            env = os.environ.copy()
            env.update(
                {"CLOUDFLARE_API_TOKEN": api_token, "CLOUDFLARE_ACCOUNT_ID": account_id}
            )
...
def get_sync_service(
    bot: discord.Client, config: Optional[Dict[str, Any]] = None
) -> IndexSyncService:
    """获取同步服务实例（单例模式）"""
