...
class SyncService:
    ...
    async def _parse_thread_data(self, thread: discord.Thread) -> Optional[dict]:
        ...
        match_time = re.search(r"原始创建时间[:：\s*]*(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日.*?(\d{2}):(\d{2})", first_msg_content)
        ...
        if match_id:
            ...
            try:
                ...
                logger.warning(f"重建帖 {thread.id} 解析作者或时间失败: {e}。中止对其的索引")
                return None
            
            # 2. 检查并解析补档链接: https://discord.com/channels/GUILD_ID/CHANNEL_ID/MESSAGE_ID
            match_url = re.search(r"补档[:：\s*]*\d+\.\s*\[.*?\]\((https://discord.com/channels/(\d+)/(\d+)/(\d+))\)", first_msg_content)
            if not match_url:
                logger.debug(f"重建帖 {thread.id} 的补档图片链接提取失败。中止对其的索引")
                return None
            ...
            try:
                ...
                if not isinstance(target_channel, discord.abc.Messageable):
                    logger.warning(f"重建帖 {thread.id} 的补档链接指向了一个非消息频道 (ID: {channel_id})。中止对其的索引")
                    return None
                ...
                if not target_message or not target_message.attachments:
                    logger.debug(f"重建帖 {thread.id} 的补档消息 ({message_id}) 不存在或没有附件。中止对其的索引")
                    return None
                
                thumbnail_url = target_message.attachments[0].url

            except (discord.NotFound, discord.Forbidden, Exception) as e:
                logger.error(f"重建帖 {thread.id} 获取补档消息时出错: {e}。中止对其的索引", exc_info=True)
                return None
            ...
            if second_msg:
                ...
                matches = re.findall(f'{emoji_pattern}\\s*(\\d+)(?=\\s|\\||$)', second_msg.content)
                
                for num_str in matches:
                    try:
                        potential_reaction_numbers.append(int(num_str))
                    except ValueError:
                        logger.warning(f"无法将重建帖 {thread.id} 中的反应数 '{num_str}' 转换为整数", exc_info=True)
                ...
                cleaning_pattern = re.compile(f"(?:{reaction_line_pattern}|{attachment_line_pattern}|{edited_line_pattern})\\n?", re.MULTILINE)
        ...
        return {
            ...
            "last_active_at": discord.utils.snowflake_time(thread.last_message_id) if thread.last_message_id else thread.created_at,
        ...
        if isinstance(thread, int):
            ...
            try:
                ...
                if not isinstance(fetched_channel, discord.Thread):
                    logger.warning(f"sync_thread: 获取到的 channel {thread_id} 不是一个帖子，将从索引中删除。")
                ...
                logger.warning(f"sync_thread: 无法找到帖子 {thread_id}，可能已被删除。将从数据库中移除。")
                async with self.session_factory() as session:
                    repo = ThreadManagerRepository(session=session)
                    await repo.delete_thread_index(thread_id=thread_id)
                return
            except Exception as e:
                logger.error(f"sync_thread: 通过ID {thread_id} 获取帖子时发生未知错误: {e}", exc_info=True)
                return
            ...
            try:
                ...
                logger.warning(f"sync_thread (fetch_if_incomplete): 无法找到帖子 {thread.id}，可能已被删除。")
                async with self.session_factory() as session:
                    repo = ThreadManagerRepository(session=session)
