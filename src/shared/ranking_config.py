...
class RankingConfig:
    ...
    def validate(cls):
        ...
        # 检查权重因子
        weight_sum = (
            cls.TIME_WEIGHT_FACTOR + cls.TAG_WEIGHT_FACTOR + cls.REACTION_WEIGHT_FACTOR
        )
        assert abs(weight_sum - 1.0) < 0.001, (
            f"三个权重因子之和必须为1，当前为{weight_sum}"
        )
        ...
        assert cls.SEVERE_PENALTY_THRESHOLD < cls.MILD_PENALTY_THRESHOLD, (
            "严重惩罚阈值必须小于轻度惩罚阈值"
        )
        print("✅ 排序算法配置验证通过")
