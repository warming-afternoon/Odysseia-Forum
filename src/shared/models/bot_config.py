...
from ..enum.search_config_type import SearchConfigType
...
class BotConfig(SQLModel, table=True):
    ...
    value_int: Optional[int] = Field(default=None, sa_column=Column(BigInteger), description="用于存储整数配置值")
    value_float: Optional[float] = Field(default=None, sa_column=Column(Float), description="用于存储浮点数配置值")
    ...
    update_user_id: Optional[int] = Field(default=None, description="最后修改此配置的用户ID")