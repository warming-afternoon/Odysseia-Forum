...
class TagNotificationTask(SQLModel, table=True):
    ...
    id: int | None = Field(
        default=None, sa_column=Column(BigInteger, primary_key=True, autoincrement=True), description="通知发送任务的内部主键"
    )
    """通知发送任务的内部主键"""

    proposal_id: int | None = Field(
        default=None, sa_column=Column(BigInteger, nullable=True, unique=True), description="关联标签申请的内部 ID；每个申请最多创建一条通知任务"
    )
    """关联标签申请的内部 ID；每个申请最多创建一条通知任务"""

    kind: str = Field(default="proposal", description="通知类型：proposal 审核，converted 转换待分类，conflict 分类冲突")
    """持久化发送任务的业务类型"""

    tag_id: int | None = Field(default=None, sa_column=Column(BigInteger), description="生命周期通知关联的内部标签 ID")
    """分类消息关联的标签实体"""
