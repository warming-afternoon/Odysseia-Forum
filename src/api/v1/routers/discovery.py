...
async def get_random_threads(
    ...
    exclude_channel_ids: Optional[List[int]] = Query(default=None, description="要排除的频道ID列表"),
    ...
    if not async_session_factory or not cache_service_instance:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="核心服务尚未初始化")
    ...
    final_exclude_channels: list[int] = [int(cid) for cid in (exclude_channel_ids or [])]
    if abyss_config:
    ...
    try:
        async with async_session_factory() as session:
            ...
            return builder.build_list(threads, collected_ids, channel_to_virtual=channel_to_virtual)
