...
class BannerApplicationRequest(BaseModel):
    """Banner申请请求模型"""
    thread_id: str = Field(..., description="帖子ID（纯数字字符串）", min_length=17, max_length=20)
...
@router.post("/apply", response_model=BannerApplicationResponse, summary="提交Banner申请")
    ...
    try:
        async with async_session_factory() as session:
            ...
            if bot_instance and banner_config and application:
                ...
                if not review_sent:
                    logger.warning(f"审核消息发送失败，但申请已创建。申请ID: {application.id}")
            else:
