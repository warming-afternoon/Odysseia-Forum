...
class SearchRequest(BaseModel):
    ...
    tag_logic: str = Field(
        default="and",
        description="多标签搜索逻辑: "
        "'and'(所有标签都匹配), 'or'(任意标签匹配)"
    )
    keywords: Optional[str] = Field(
        default=None,
        description="搜索关键词，使用逗号分隔 AND 逻辑，"
        "斜杠分隔 OR 逻辑"
    )
    ...
    created_after: Optional[str] = Field(
        ...
        "(格式: YYYY-MM-DD 或相对时间如 -7d)"
    )
    created_before: Optional[str] = Field(
        default=None,
        description="发帖时间早于此日期 "
        "(格式: YYYY-MM-DD 或相对时间如 -7d)"
    )
    active_after: Optional[str] = Field(
        default=None,
        description="最后活跃时间晚于此日期 "
        "(格式: YYYY-MM-DD 或相对时间如 -7d)"
    )
    active_before: Optional[str] = Field(
        default=None,
        description="最后活跃时间早于此日期 "
        "(格式: YYYY-MM-DD 或相对时间如 -7d)"
    )
    ...
    limit: int = Field(
        ...
        ge=1,
        le=100,
        description="每页返回的结果数量 (范围: 1-100)"
    )
    offset: int = Field(
        default=0,
        ge=0,
